...
if (isset($_POST['verify_order']) && $_POST['verify_order'] == 'yes') {
    ...
    // Verify that the order exists, belongs to the user, and has "delivered" status
    $check_sql = "SELECT id FROM orders WHERE id = ? AND user_id = ? AND order_status = 'delivered'";
    ...
    if ($check_result->num_rows > 0) {
        ...
        $update_sql = "UPDATE orders SET order_status = 'completed', updated_at = NOW() WHERE id = ?";
